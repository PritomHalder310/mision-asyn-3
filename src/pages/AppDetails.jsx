...
import { useParams, Navigate } from "react-router-dom"; // 🔥 add Navigate
...
const AppDetails = () => {
  ...
  const handleInstall = () => {
    ...
    const alreadyInstalled = storedApps.find(
      (a) => a.id === app.id
    );
    ...
    const updatedApps = [...storedApps, appData];
      ...
      <div className="flex items-center gap-6 border p-6 rounded-xl">
        ...
        <div className="flex-1">
          ...
          <div className="flex gap-10 mt-4">
            ...
            <div>
              <p className="text-orange-500 text-sm">⭐ Average</p>
              <h3 className="font-bold text-lg">{app.ratingAvg}</h3>
            </div>

            <div>
              <p className="text-purple-500 text-sm">📊 Reviews</p>
              <h3 className="font-bold text-lg">{app.reviews}</h3>
            </div>
          ...
          <button
            ...
            disabled={installed}
            className={`mt-4 px-5 py-2 rounded-md text-white text-sm ${
              installed
